...
export const getBookByAbbreviation = (
  bible: EBible,
  book: string
): IBook | undefined => {
  return getBible(bible).books.find((item) => item.abbreviation === book);
};
...
export const getVerses = (
  ...
  return getBible(bible).verses.filter(
    (item) => bookId === item.book_id && chapterId === item.chapter
  );
};
