...
const Member: NextPage = () => {
  ...
  const [userAvailability, setUserAvailability] = useState<
    { uuid: string; availability: "unavailable" | "available" | "unknown" }[]
  >([]);
  ...
  useEffect(() => {
    const availability: {
      uuid: string;
      availability: "unavailable" | "available" | "unknown";
    }[] = [];
  ...
  const checkOptionAvailability = (
    uuid: string,
  ): "unavailable" | "available" | "unknown" | undefined => {
    const find = userAvailability?.find((item) => item.uuid === uuid);
    ...
    <>
      ...
      >
        <>
          ...
          <Container>
            <p>
              Veja, acompanhe e preencha as escalas dos ministérios que você
              participa.
            </p>
            <List>
              {team?.map((item) => (
      ...
      <Modal
        onClose={() => setSelectedRotation(undefined)}
        isOpen={!!selectedRotation}
      >
        <ModalOpen bottom={deviceInfo?.bottom}>
          <div>
            <h1>Escala</h1>
            <h2>{selectedRotation?.title}</h2>
            <p>
              {descriptionMessage[selectedRotation?.status || "open"] || ""}
            </p>
          </div>
          <ListItems>
            {selectedRotation?.items
              .sort((a, b) => {
                ...
                <div key={item.uuid}>
                  <HeaderItem>
                    <div>
                      <div>
                        {item.rotationDate &&
                          DateUtils.formatDateDayAndMonth(item.rotationDate)}
                      </div>
                      <div>
                        {item.rotationDate &&
                          DateUtils.getWeekdayStr(item.rotationDate)}
                      </div>
                      <div>
                        {item.rotationDate &&
                          DateUtils.formatTime(item.rotationDate)}
                      </div>
                    </div>
                    <div
                      className={`buttonsActions ${verifyIfLoggedUserWasSelected(item.rotationItemMembers || []) ? "selected" : ""}`}
                    >
                      <button
                        disabled={
                          !!verifyIfLoggedUserWasSelected(
                            item.rotationItemMembers || [],
                          )
                        }
                        className={`${checkOptionAvailability(item.uuid) === "available" && "active"}`}
                        onClick={() =>
                          handleAvailability(item.uuid, "available")
                        }
                      >
                        <FiCheck /> Sim
                      </button>
                      <button
                        ...
                        className={`${checkOptionAvailability(item.uuid) === "unavailable" && "active"}`}
                        onClick={() =>
                          handleAvailability(item.uuid, "unavailable")
                        }
                      >
                        <FiX /> Não
                      </button>
                      <button
                        ...
                        className={`${checkOptionAvailability(item.uuid) === "unknown" && "active"}`}
                        onClick={() => handleAvailability(item.uuid, "unknown")}
                      >
                        Talvez
                      </button>
                    </div>
                  ...
                  {selectedRotation.status !== "open" && (
                    <MembersSelecteds>
                      {item.rotationItemMembers?.map((member) => (
                        <div
                          key={member.uuid}
                          className={`${verifyIfLoggedUser(member.member.uuid) ? "active" : ""}`}
                        >
                          <div>Responsável</div>
