...
const Preaches: NextPage<IEventsPage> = ({ data }) => {
  ...
  const handleGetAll = async () => {
    ...
    const response = await apiLocal.getContents(paginator.page + 1, 20, true, 'preach')
  ...
  return (
    <Website title='Exposições' changeScroll={changeScroll} openMenu={open} toggleMenu={toggleMenu}>
      <>
...
export async function getStaticProps() {
  const api = new Api()
  const data = await api.getContents(1, "preach", 20, true)
